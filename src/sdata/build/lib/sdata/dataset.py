...
def dict_collation_fn(samples: List, combine_tensors: bool = True, combine_scalars: bool = True) -> Dict:
...
def create_loader(
    ...
    # batching
    datapipeline = datapipeline.batched(batch_size, partial=partial, collation_fn=collation_fn)

    # apply transforms which act on batched samples
    for i, trf in enumerate(batched_transforms):
        trf = instantiate(trf)
        if isinstance(trf, AbstractFilter):
            print(f"Adding filter {trf.__class__.__name__} as batched transform #{i} " f"to the datapipeline")
            datapipeline = datapipeline.select(trf)
        elif isinstance(trf, AbstractMapper):
            print(f"Adding mapper {trf.__class__.__name__} as batched transform #{i} " f"to the datapipeline")
    ...
    # create loader
    loader = torch.utils.data.DataLoader(datapipeline, batch_size=None, num_workers=num_workers, **loader_kwargs)
    return loader
...
def custom_decoder(key, data):
    ...
    if key.endswith('mp4'):
        data = wds.torch_video("mp4", data)
    elif key.endswith('wav'):
        data = wds.torch_audio("wav", data)
    elif key.endswith('pt'):
        data = wds.torch_loads(data)
    else:
...
def create_dataset(
    ...
    for i, prepro_config in enumerate(preprocessors):
        ...
        if isinstance(prepro, AbstractFilter):
            print(f"Adding filter {prepro.__class__.__name__} as preprocessor #{i} " f"to the datapipeline")
            datapipeline = datapipeline.select(prepro)
        elif isinstance(prepro, AbstractMapper):
            print(f"Adding mapper {prepro.__class__.__name__} as preprocessor #{i} " f"to the datapipeline")
    ...
    for decoder_spec in decoders:
        if isinstance(decoder_spec, (Dict, DictConfig)):
            ...
            assert (
                ...
                or decoder_spec == "video"
                or decoder_spec == "custom"
            ), ("when decoder is specified via a string, then it has to be a a " "decoder known to webdataset")
            print(f"Adding decoder {decoder_spec} to decoders.")
            if decoder_spec == "video":
                prepared_decoders.append(wds.torch_video)
            elif decoder_spec == "custom":
                prepared_decoders.append(custom_decoder)
            else:
    ...
    for i, postro_config in enumerate(postprocessors):
        ...
        if isinstance(postpro, AbstractFilter):
            print(f"Adding filter {postpro.__class__.__name__} as postprocessor #{i} " f"to the datapipeline")
            datapipeline = datapipeline.select(postpro)
        elif isinstance(postpro, AbstractMapper):
            print(f"Adding mapper {postpro.__class__.__name__} as postprocessor #{i} " f"to the datapipeline")
