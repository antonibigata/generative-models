...
srun python main.py --base configs/example_training/svd_interpolation.yaml --wandb True lightning.num_nodes 8 \
    lightning.strategy=deepspeed_stage_2_offload lightning.trainer.precision=32 model.base_learning_rate=1.-4 \
    data.params.train.datapipeline.filelist=/fsx/rs2517/data/lists/HDTF/filelist_videos_train.txt \
    lightning.trainer.devices=8 lightning.trainer.accumulate_grad_batches=2 data.params.train.loader.batch_size=2 