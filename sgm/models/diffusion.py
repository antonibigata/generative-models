...
from ..util import default, disabled_train, get_obj_from_str, instantiate_from_config, log_txt_as_img, save_image
...
class DiffusionEngine(pl.LightningModule):
	def __init__(
		...
		calculate_landmarks: Optional[bool] = False,
		what_mask: str = 'full',
	):
		super().__init__()
	...
	"Face alignment does not support multiple gpus"
	# def on_fit_start(self):
	# 	if self.calculate_landmarks:
	# 		self.lmks_detector = face_alignment.FaceAlignment(
	...
	def init_from_ckpt(self, path: str, remove_keys_from_weights: Optional[Union[List, Tuple]] = None, print_missing_keys = True) -> None:
		...
		# for k in list(sd.keys()):
		# 	if 'model.diffusion_model.input_blocks.0.0.weight' in k:
		# 		value = sd[k]
		# 		print(k, value.shape)
		# 		# Slicing the tensor
		# 		# slice1 = value[:, :4, :, :]
		# 		# slice2 = value[:, 4:, :, :]
		# 		# are_equal = torch.equal(slice1, slice2)
		# 		# print(are_equal, torch.sum(torch.abs(slice1)), torch.sum(torch.abs(slice2)))
	...
	def calculate_landmarks_masks(self, video_frames):

		B, T, H, W, C = video_frames.shape
		video_frames = video_frames.view(-1, *video_frames.shape[2:])
		landmarks_list = []
		for i in range(video_frames.shape[0]):
			img_tmp = video_frames[i].clone()
			img_tmp = img_tmp.cpu().numpy()
			land = self.lmks_detector.get_landmarks(img_tmp)[0]
		...
		# landmarks_list = landmarks_list.reshape(B, T, *landmarks_list.shape[1:])
		
		original_size = (H,W)
		target_size = (64, 64)
		if self.what_mask == "full":
			mask = create_masks_from_landmarks_full_size(landmarks_list, original_size[0], original_size[1], offset=-0.01)
		elif self.what_mask == "box":
			mask = create_masks_from_landmarks_box(landmarks_list, (original_size[0], original_size[1]), box_expand=0.0)
		else:
			mask = create_face_mask_from_landmarks(landmarks_list, original_size[0], original_size[1], mask_expand=0.05)		
		mask = F.interpolate(mask.unsqueeze(1).float(), size=target_size, mode="nearest") # B x 14 x 64 x 64
		mask = mask.reshape(B, T, *mask.shape[1:]) # B x 1 x 14 x 224 x 224
		mask = rearrange(mask, "b t c h w -> b c t h w")
		mask = mask.to(self.device)
		return landmarks_list, mask

	def shared_step(self, batch: Dict) -> Any:
		x = self.get_input(batch)
		# dict_keys(['audio_emb', 'raw_audio', 'latents', 'cond_frames', 'cond_frames_without_noise', 'cond_aug', 'gt', 'masks', 'masks_full_image', 'motion_bucket_id', 'fps_id', 'num_video_frames', 'image_only_indicator', 'video_frames'])
		
		if self.input_key != "latents": # this is when x is an image
			x = self.encode_first_stage(x)
		batch["global_step"] = self.global_step

		# print(batch['masks'].shape) # b x c x t x h x w -> torch.Size([1, 1, 14, 64, 64])
		if self.calculate_landmarks:
			# compute landmarks and masks while training
			t0 = time.time()
			landmarks, masks = self.calculate_landmarks_masks(batch['video_frames'])
			print('Time to compute landmarks {:.3f}'.format(time.time()-t0))
			batch['masks'] = masks
