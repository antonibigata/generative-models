...
class VideoDataset(Dataset):
	def __init__(
		...
		self.audio_emb_path = audio_emb_path
		...
		with open(filelist, "r") as files:
			for f in files.readlines():
				...
				if dataset_path is not None: # vox
					video_file = os.path.join(dataset_path, f)
				else:
					video_file = f
				   
				self.filelist += [video_file]
				if audio_path is not None:
					self.audio_filelist += [audio_path]

				if self.get_masks:
					if self.landmarks_path is not None:
						# Vox landmarks end with .pkl
						head, tail = os.path.split(f)
						landmark_path = os.path.join(self.landmarks_path, head, tail).replace(video_extension, ".pkl")
					else:
						# HDTF landmarks are on the same folder with videos
		...
		if load_all_possible_indexes:
			...
			if len(self.audio_filelist) > 0:
				...
				if self.get_masks:
					self._indexes = list(zip(self.filelist, [None] * len(self.filelist), self.landmark_filelist))
				else:
					self._indexes = list(zip(self.filelist, [None] * len(self.filelist), [None] * len(self.filelist)))
			
			print("Indexes", len(self._indexes), "\n")
	...
	def _load_landmarks(self, filename, original_size, target_size, indexes):
		...
		if file_extension == '.npy':
			...
			with open(filename, 'rb') as file:
				...
				landmarks = np.asarray(data['landmarks']) #[indexes, :]
		...
		if self.what_mask == "full":
			...
			mask = create_masks_from_landmarks_box(landmarks, (original_size[0], original_size[1]), nose_index = 29, box_expand=0.0)
		elif self.what_mask == "half":
			mask = create_masks_half_face((original_size[0], original_size[1]), self.num_frames)
		elif self.what_mask == "cheeks":
			mask = create_masks_from_landmarks_cheeks(landmarks, (original_size[0], original_size[1]), box_expand=0.0)
		else:
		...
		return mask_full.unsqueeze(1).float(), mask
	...
	def _get_frames_and_audio(self, idx):
		...
		if self.audio_in_video:
			...
			frames_video = vr.get_batch(indexes)
		...
		if self.use_latent and self.precomputed_latent:
			...
			if self.latent_path is not None:
				latent_file = video_file.replace(self.video_ext, f"_{self.latent_type}_512_latent.safetensors").replace(
					self.dataset_path, self.latent_path
					)
			else:
		...
		if not self.from_audio_embedding:
			...
			if audio_file is not None:
				audio_emb_path = audio_file.split(".")[0] + f"_{self.audio_emb_type}_emb.pt"
			else:
				audio_emb_path = video_file.replace(self.video_ext, f"_{self.audio_emb_type}_emb.pt").replace(
					self.dataset_path, self.audio_emb_path
				)  
			audio = torch.load(audio_emb_path)
			audio_frames = audio[indexes, :] # num_frames x 2 x 768
		...
		if self.get_masks:
			...
			masks_full_image, masks = self._load_landmarks(land_file, original_size, target_size, indexes)
			masks = masks.permute(1, 0, 2, 3)
			# print(masks_full_image.shape, masks.shape) # torch.Size([14, 1, 652, 652]) torch.Size([1, 14, 64, 64])
			
			resized_masks = F.interpolate(masks, size=(noisy_cond.shape[-2], noisy_cond.shape[-1]), mode="nearest")
			masks_full_image = F.interpolate(masks_full_image, size=(self.resize_size, self.resize_size), mode="nearest")
			# masks_full_image = masks_full_image.permute(1, 0, 2, 3) # 1 x 14 x 512 x 512
			if not self.only_predict_mouth:
				noisy_cond = noisy_cond * (1 - resized_masks)
			
		else:
			if self.what_mask == "half":
				target_size = (
					(self.resize_size, self.resize_size)
					if not self.use_latent
					else (self.resize_size // 8, self.resize_size // 8)
				)
				masks = create_masks_half_face(original_size, self.num_frames)
				masks_full_image = masks.clone()
				masks_full_image = F.interpolate(masks_full_image, size=(self.resize_size, self.resize_size), mode="nearest")
				masks_full_image = rearrange(masks_full_image, "t c h w -> c t h w") 
				# grid = torch_utils.save_image(
				#     masks.float(),
				#     './masks.png',
				#     normalize=True,
				# )				
				masks = F.interpolate(masks.float(), size=target_size, mode="nearest")
				masks = rearrange(masks, "t c h w -> c t h w") 
			else:
				masks = torch.zeros_like(noisy_cond)
				masks[:, :, noisy_cond.shape[-2] // 2 :, :] = 1
				if not self.only_predict_mouth:
					noisy_cond = noisy_cond * (1 - masks)
				masks_full_image = None

		# print('clean_cond', clean_cond.shape) # 3 x 1 x 512 x 512 Reference frame
		# print('noisy_cond', noisy_cond.shape) # # 4 x 1 x 64 x 64 Latent Reference frame
		# print('target', target.shape) # 4 x 14 x 64 x 64 frames 
		# print('masks', masks.shape) # 4 x 1 x 64 x 64 hdtf 1, 14, 64, 64
		# print('audio_frames', audio_frames.shape) # 14 x 2 x 768
		# print('raw_audio', raw_audio.shape) # 8960: 14 x 640
		# print('cond_noise' , cond_noise) # 0.0
		# frames_video 14 x 224 x 224 x 3

		return clean_cond, noisy_cond, target, masks, masks_full_image, audio_frames, raw_audio, cond_noise, frames_video

	def _get_indexes(self, video_filelist, audio_filelist, landmark_filelist=None):
		indexes = []
		self.og_shape = None
		if len(audio_filelist) > 0:
			...
			for i, (vid_file) in enumerate((video_filelist)):
				vr = decord.VideoReader(vid_file)
				if self.og_shape is None:
					self.og_shape = vr[0].shape[-2]
				len_video = len(vr)
				...
				if len_video < self.num_frames:
					...
					possible_indexes = list(sliding_window(range(len_video), self.num_frames))[:: self.step]
					possible_indexes = list(map(lambda x: (x, vid_file), possible_indexes))
					indexes.extend(possible_indexes)
			print("Indexes", len(indexes), "\n")
		return indexes
	...
	def __getitem__(self, idx):
		try:
			clean_cond, noisy_cond, target, masks, masks_full_image, audio, raw_audio, cond_noise, frames_video = self._get_frames_and_audio(idx)
		...
		if masks_full_image is not None:
			out_data["masks_full_image"] = masks_full_image
		...
		out_data['video_frames'] = frames_video
		return out_data
