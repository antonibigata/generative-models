...
from sgm.modules.lipreader.espnet.nets.pytorch_backend.nets_utils import to_device
...
class CTC(torch.nn.Module):
    ...
    def __init__(self, odim, eprojs, dropout_rate, ctc_type="builtin", reduce=True):
        ...
        self.ctc_type = ctc_type if LooseVersion(torch.__version__) < LooseVersion("1.7.0") else "builtin"
    ...
    def forward(self, hs_pad, hlens, ys_pad):
        ...
        if self.ctc_type == "builtin":
            ...
            if self.ctc_type == "gtnctc":
                ...
                ys_true = ys
            self.loss = to_device(hs_pad, self.loss_fn(ys_hat, ys_true, hlens, olens)).to(dtype=dtype)
    ...
    def forced_align_batch(self, hs_pad, ys_pad, ilens, blank_id=0):
        ...
        for t in range(1, Tmax):
            ...
            logdelta[t, :, s_start:s_end][nignore] = (
                candidates_[b_i, s_i, idx][nignore] + lpz[t, b_i, ys_int[:, s_start:s_end]][nignore]
            )
