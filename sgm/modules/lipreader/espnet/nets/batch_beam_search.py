...
from sgm.modules.lipreader.espnet.nets.beam_search import BeamSearch, Hypothesis
...
class BatchBeamSearch(BeamSearch):
    ...
    def _batch_select(self, hyps: BatchHypothesis, ids: List[int]) -> BatchHypothesis:
        return BatchHypothesis(
            ...
            states={k: [self.scorers[k].select_state(v, i) for i in ids] for k, v in hyps.states.items()},
        )
    ...
    def unbatchfy(self, batch_hyps: BatchHypothesis) -> List[Hypothesis]:
        ...
        return [
            Hypothesis(
                ...
                states={k: v.select_state(batch_hyps.states[k], i) for k, v in self.scorers.items()},
            )
    ...
    def search(self, running_hyps: BatchHypothesis, x: torch.Tensor) -> BatchHypothesis:
        ...
        # partial scoring
        if self.do_pre_beam:
            pre_beam_scores = weighted_scores if self.pre_beam_score_key == "full" else scores[self.pre_beam_score_key]
    ...
    def post_process(
        ...
        if self.token_list is not None:
            logging.debug(
                "best hypo: " + "".join([self.token_list[x] for x in running_hyps.yseq[0, 1 : running_hyps.length[0]]])
            )
