...
from sgm.modules.lipreader.espnet.nets.e2e_asr_common import end_detect
from sgm.modules.lipreader.espnet.nets.scorer_interface import PartialScorerInterface, ScorerInterface
...
class BeamSearch(torch.nn.Module):
    ...
    def __init__(
        ...
        self.do_pre_beam = (
            self.pre_beam_score_key is not None and self.pre_beam_size < self.n_vocab and len(self.part_scorers) > 0
        )
    ...
    def search(self, running_hyps: List[Hypothesis], x: torch.Tensor) -> List[Hypothesis]:
        ...
        for hyp in running_hyps:
            ...
            if self.do_pre_beam:
                pre_beam_scores = (
                    weighted_scores if self.pre_beam_score_key == "full" else scores[self.pre_beam_score_key]
                )
    ...
    def forward(self, x: torch.Tensor, maxlenratio: float = 0.0, minlenratio: float = 0.0) -> List[Hypothesis]:
        ...
        for k, v in best.scores.items():
            logging.debug(f"{v:6.2f} * {self.weights[k]:3} = {v * self.weights[k]:6.2f} for {k}")
        ...
        if self.token_list is not None:
            logging.debug("best hypo: " + "".join([self.token_list[x] for x in best.yseq[1:-1]]) + "\n")
        return nbest_hyps
