...
class ReferenceAttentionControl:
    ...
    def register_reference_hooks(
        ...
        if do_classifier_free_guidance:
            ...
            uc_mask = torch.Tensor([0] * batch_size * num_images_per_prompt * 2).to(device).bool()
        ...
        def hacked_basic_transformer_inner_forward(
            ...
            if self.only_cross_attention:
                ...
                if MODE == "read":
                    ...
                    modify_norm_hidden_states = torch.cat([norm_hidden_states] + bank_feas, dim=1)
    ...
    def update(self, writer, do_classifier_free_guidance=False, dtype=torch.float16):
        if self.reference_attn:
            if self.fusion_blocks == "midup":
                ...
                reader_attn_modules = [
                    module for module in torch_dfs(self.unet) if isinstance(module, TemporalBasicTransformerBlock)
                ]
                writer_attn_modules = [
                    module for module in torch_dfs(writer.unet) if isinstance(module, BasicTransformerBlock)
                ]
    ...
    def read(self, do_classifier_free_guidance=False, dtype=torch.float16):
        if self.fusion_blocks == "midup":
            writer_attn_modules = [
                module
                for module in (torch_dfs(self.unet.mid_block) + torch_dfs(self.unet.up_blocks))
        ...
        for w in writer_attn_modules:
            for v in w.bank:
                if do_classifier_free_guidance:
                    out_bank.append(torch.cat([v, v]).to(dtype))
                else:
                    out_bank.append(v.clone().to(dtype))
