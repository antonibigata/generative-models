...
srun python main.py --base configs/example_training/svd_keyframes_reference.yaml --wandb True lightning.trainer.num_nodes 8 \
    ...
    lightning.trainer.devices=4 lightning.trainer.accumulate_grad_batches=1 data.params.train.datapipeline.virtual_increase=1 \
    model.params.network_config.params.audio_cond_method=both_keyframes data.params.train.loader.batch_size=2 \
    model.params.loss_fn_config.params.lambda_lower=2. model.params.network_config.params.skip_time=False \
    model.params.network_config.params.reference_to=self_extra