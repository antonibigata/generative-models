...
#SBATCH --no-requeue
#SBATCH --account all
#SBATCH --exclude=a100-st-p4d24xlarge-53,a100-st-p4d24xlarge-61
...
srun python main.py --base configs/example_training/svd_keyframes_emo_cross.yaml --wandb True lightning.trainer.num_nodes 8 \
    lightning.strategy=deepspeed_stage_1 lightning.trainer.precision=32 model.base_learning_rate=1.e-5 \
    data.params.train.datapipeline.filelist=/data/home/antoni/datasets/HDTF/filelist_train.txt \
    ...
    model.params.loss_fn_config.params.lambda_lower=2. data.params.train.datapipeline.virtual_increase=1000 \
    data.params.train.datapipeline.select_randomly=False 'model.params.to_freeze=["time_"]' 'model.params.to_unfreeze=["time_embed"]' \
    data.params.train.datapipeline.balance_datasets=False  \
    model.params.ckpt_path=logs/2024-09-10T13-46-16_example_training-svd_keyframes_emo_cross/checkpoints/last.ckpt/checkpoint/mp_rank_00_model_states.pt 'model.params.remove_keys_from_weights=[]' \