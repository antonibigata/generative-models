    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if "CUDA out of memory" in str(e):
                print(f"CUDA OOM encountered in {func.__name__}: Moving computation to CPU.")
...
class AudioWrapper(nn.Module):
    ...
    def calculate_splits(self, tensor, min_last_size):
        ...
        split_sizes.append(total_size - sum(split_sizes))  # Ensure the last split has at least min_last_size
    ...
    def wav2vec2_encoding(self, audio_frames, chunks=None):
        ...
        if audio_embeddings.shape[1] % 2 != 0:
            audio_embeddings = torch.cat([audio_embeddings, torch.zeros_like(audio_embeddings[:, :1])], dim=1)
