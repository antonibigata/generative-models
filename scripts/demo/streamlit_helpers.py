...
def get_guider(options, key):
    guider = st.sidebar.selectbox(
        ...
        ["VanillaCFG", "IdentityGuider", "LinearPredictionGuider", "TrianglePredictionGuider", "MultipleCondVanilla"],
    ...
    elif guider == "MultipleCondVanilla":
        scales = st.text_input("Enter scales (comma-separated, e.g., 0.5, 1.5, 2.5):")

        try:
            # Convert the input string to a list of floats
            scales = [float(x.strip()) for x in scales.split(",")]
            st.write("Entered scales:", scales)
        except ValueError:
            # Handle cases where conversion to float fails (e.g., invalid input)
            if scales:  # Only show the error if there's some input
                st.error("Please make sure all entries are valid numbers.")

        condition_names = st.text_input("Enter condition names (comma-separated, e.g., 'txt, image_embeds'):")
        condition_names = [x.strip() for x in condition_names.split(",")]
...
def get_discretization(discretization, options, key=1):
    ...
    elif discretization == "AYSDiscretization":
        discretization_config = {
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                if gt is not None:
                    # gt = rearrange(gt, "b c h w -> b () c h w")
                    samples = torch.cat([samples, torch.clamp((gt + 1.0) / 2.0, min=0.0, max=1.0)], dim=-1)
                if T is None:
...
def get_batch(
    ...
    if T is not None:
        ...
        batch["num_video_frames"] = value_dict[key]
